...
void EV_Update(EV_HandleTypeDef *ev, float dt)
{
    ...
    if (ev->brake_pedal > EV_REGEN_THRESHOLD_PCT) {
        ev->regen_level  = ev->brake_pedal * 0.7f;   /* 0–70 % scale */
        ev->motor_torque = -(ev->regen_level * EV_REGEN_TORQUE_MAX_NM / 100.0f)
                           * 100.0f;
        /* Simplified: torque = -regen_pct * max_regen_Nm */
        ev->motor_torque = -(ev->brake_pedal * 0.7f) / 100.0f
                           * EV_REGEN_TORQUE_MAX_NM;
    }

    /* ── 3. Speed — simple inertia model (Euler) ──────────────────────────── */
    /*    accel (m/s²) = (net_torque - drag) / mass_factor                    */
    float drag  = ev->speed_kmh * 0.01f;       /* simple aerodynamic drag    */
    float accel = (ev->motor_torque - drag) / 150.0f;

    /* Convert m/s² × dt → Δkm/h  (×3.6 for m/s → km/h) */
    ev->speed_kmh = CLAMP(ev->speed_kmh + accel * dt * 3.6f,
                          0.0f, EV_MAX_SPEED_KMH);
    ...
    /* ── 5. SOC integration ───────────────────────────────────────────────── */
    /* ΔSOC = (P_kW × dt_s) / (capacity_kWh × 3600) × 100 %                 */
    //float delta_soc = (ev->power_kw * dt)
    //                  / (EV_BATTERY_CAPACITY_KWH * 3600.0f)
    ///                  * 100.0f;
   // float delta_soc = (ev->power_kw * dt) / (EV_BATTERY_CAPACITY_KWH * 10.0f);
    float delta_soc = (ev->power_kw * dt)
                      / (EV_BATTERY_CAPACITY_KWH * 3600.0f)
                      * 100.0f
                      * EV_SIM_SCALE;
